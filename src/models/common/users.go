...
type User struct {
	...
	AppKey     string `orm:"column(appkey);size(128);null"`
	Address    string `orm:"column(address);size(128);null"`
	PayPass    string `orm:"column(paypass);size(64)`
	IsUsed     bool   `orm:"column(isdel);null"`
...
func GetUser(u *User, col ...string) error {
	o := orm.NewOrm()
	return o.Read(u, col...)
}
...
func UpdateUserById(m *User, cols ...string) (err error) {
	o := orm.NewOrm()
	v := User{Id: m.Id}
